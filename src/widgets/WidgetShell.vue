...
const resolved = computed(() =>
  selected.value === "auto"
    ? (props.widget.result?.suggestedPresentations[0] ??
...
async function duplicate() {
  menu.value = false;
  try {
    const copy = await store.createWidget({
      ...props.widget.invocation,
      title: `${props.widget.title} copy`.slice(0, 120),
      presentation: props.widget.presentation.type,
      width: props.widget.width,
    });
    await store.transformWidget(copy.id, { ...props.widget.presentation });
  } catch (e) {
    error.value = (e as Error).message;
  }
}
  ...
  >
    <header class="widget-header">
      ...
      <div class="widget-heading">
        <h2>{{ widget.title }}</h2>
        <p>
          {{ source.api.name }}<span class="source-dot">·</span
          >{{
            widget.invocation.mode === "sample" ? "Sample data" : "Live request"
          }}
        </p>
      </div>
      <button
      ...
      <div class="menu-anchor">
        ...
        <div v-if="menu" class="widget-menu" @keydown.esc="menu = false">
          ...
          ><button
            v-for="view in ['Data', 'Request', 'Response', 'Code']"
            :key="view"
            @click="
              tab = view;
              menu = false;
    ...
    <div class="widget-content">
      ...
      <div v-else-if="tab === 'Presentation'" class="presentation-settings">
        ...
        ><label
          ...
          >
            ...
            <option
              v-for="f in widget.result?.fields"
              :key="f.key"
              :value="f.key"
            >
              {{ f.label }}
      ...
      <TableBlock
        v-else-if="tab === 'Data' && widget.result"
        :rows="rowsOf(widget.result.data)"
        :fields="widget.result.fields"
      />
      <BlockRenderer
        v-else-if="widget.result"
