...
const name = (value: unknown) =>
  record(value) ? (value.displayName ?? value.fullName ?? value.name) : value;

/** Shape checks, not hostname checks. Explicit response paths always take priority. */
export function scenarioCollection(
  raw: Row,
): { data: unknown; collectionPath: string } | undefined {
  if (record(raw.team) && record(raw.team.record))
    return { data: raw.team, collectionPath: "team" };
  if (record(raw.record) && (raw.displayName || raw.name))
    return { data: raw, collectionPath: "$" };
  if (
    Array.isArray(raw.dates) &&
    raw.dates.every((d) => record(d) && Array.isArray(d.games))
  )
    return {
      data: raw.dates.flatMap((d) => (d as Row).games as unknown[]),
      collectionPath: "dates[].games[]",
    };
  if (scenarioFor(raw).kind) return { data: raw, collectionPath: "$" };
  for (const key of [
    "events",
    "games",
    "articles",
...
export function scenarioFor(
  ...
  pick(
    "image_url",
    ["image", "thumbnail", "photo", "avatar", "logos[0].href", "images[0].url"],
    (v) => (record(v) ? v.url : v),
  );
  ...
  if (
    requestedKind === "news" &&
    values.title &&
    (values.url || values.description || values.time)
  ) {
  ...
  if (
    requestedKind === "person" &&
    values.title &&
    (row.role || row.organization || row.email)
  ) {
    pick("role", ["jobTitle"]);
    pick("organization", ["company", "worksFor"], name);
    pick("email", []);
    return { kind: "person", values, mappings };
  }
  const schemaType = String(row["@type"] ?? row.type ?? "").toLowerCase();
  const competition =
    Array.isArray(row.competitions) && record(row.competitions[0])
      ? row.competitions[0]
      : row;
  const competitors = Array.isArray(competition.competitors)
    ? competition.competitors.filter(record)
    : [];
  ...
  if (home && !values.home_team) {
    values.home_team = name(home.team ?? home.athlete);
    mappings.home_team = `${competition === row ? "" : "competitions[0]."}competitors[${competitors.indexOf(home)}].team.displayName`;
  }
  if (away && !values.away_team) {
    values.away_team = name(away.team ?? away.athlete);
    mappings.away_team = `${competition === row ? "" : "competitions[0]."}competitors[${competitors.indexOf(away)}].team.displayName`;
  }
  if (text(values.home_team) && text(values.away_team)) {
    ...
    for (const [slot, competitor] of [
      ["home_score", home],
      ["away_score", away],
    ] as const) {
      const score = record(competitor?.score)
  ...
  const records =
    record(row.record) && Array.isArray(row.record.items)
      ? row.record.items.filter(record)
      : [];
  const total =
    records.find((r) => r.type === "total" || r.name === "overall") ??
    records[0];
  const stats = Array.isArray(total?.stats)
    ? total.stats.filter(record)
    : Array.isArray(row.stats)
      ? row.stats.filter(record)
      : [];
  ...
  for (const field of ["wins", "losses", "ties", "rank", "streak"]) {
    ...
    if (
      ...
      stat &&
      scalar(stat.displayValue ?? stat.value)
    )
      values[field] = stat.displayValue ?? stat.value;
  }
  if (
    values.record_summary ||
    records.length ||
    (stats.length && text(row.summary)) ||
    (values.wins !== undefined && values.losses !== undefined)
  ) {
    if (!values.record_summary && text(row.summary))
      values.record_summary = row.summary;
    if (!values.title && text(row.type))
      values.title =
        row.type === "total"
          ? "Overall record"
          : row.type === "road"
  ...
  pick(
    ...
    (v) =>
      record(v)
        ? [
            v.streetAddress ?? v.address1,
            v.addressLocality ?? v.city,
            v.addressRegion ?? v.state,
            v.postalCode,
            name(v.addressCountry),
          ]
            .filter(scalar)
            .join(", ")
        : v,
  );
  if (!values.address && text(row.streetAddress))
    values.address = [
      row.streetAddress,
      row.addressLocality ?? row.city,
      row.addressRegion ?? row.state,
      row.postalCode,
    ]
      .filter(scalar)
      .join(", ");
  ...
  if (
    schemaType.includes("event") ||
    (values.time && (row.startDate || row.endDate || values.venue))
  ) {
  ...
  if (
    schemaType.includes("person") ||
    row.jobTitle ||
    (row.firstName && row.lastName)
  ) {
    if (!values.title && row.firstName && row.lastName)
      values.title = `${row.firstName} ${row.lastName}`;
  ...
  if (
    ...
    (values.title && (row.price !== undefined || record(row.offers)))
  ) {
  ...
  if (
    ...
    (values.title &&
      ...
      (row.headline ||
        ...
        row.published ||
        row.datePublished ||
        (row.date && row.link && values.description)))
  ) {
    pick("author", ["creator", "byline"], name);
    pick("publisher", ["source", "provider"], name);
    return { kind: "news", values, mappings };
  ...
  if (
    ...
    return { kind: "places", values, mappings };
